...
public class nhom12 extends Applet
{	
	//CLA
	...
	public static byte lenSODU = (byte)0;
	...
	private final static byte INS_DEPOSIT = (byte) 0x62;
	private final static byte INS_PAYMENT = (byte) 0x63;
	...
	*****/
			...
			for(short i=0;i<=lenData;i++){
				...
				if(flag == (short)1){
					...
					if(lenID == (byte)0){
						...
						lenADDRESS = (byte)(objDatalen);
						objData = new byte[0];
					}
					 else if(lenSODU == (byte)0){
						if(objDatalen > 25){
							 ISOException.throwIt(ISO7816.SW_WRONG_LENGTH);
						 }
						 byte[] temp = encrypt(objData);
	...
	private void OutputInformation(APDU apdu, byte[] buffer){
		apdu.setIncomingAndReceive();
			if(buffer[ISO7816.OFFSET_P1] == OUT_ID){
				...
				Util.arrayCopy(decrypt(OpSoDu,lenSODU),(short)0,buffer,(short)0,lenSODU);
    ...
    // // Luu giá tri so du hien tai truoc khi thuc hien giao dich
		// byte[] prevEncryptedBalance = JCSystem.makeTransientByteArray((short) lenSODU, JCSystem.CLEAR_ON_DESELECT);
		// Util.arrayCopyNonAtomic(OpSoDu, (short) 0, prevEncryptedBalance, (short) 0, lenSODU);
    // // Giai mã so du
		// byte[] decryptedBalance = decrypt(prevEncryptedBalance, lenSODU);
    // // Luy giá tri so du hien tai tu mang  giai ma
		// short prevBalance = Util.getShort(decryptedBalance, (short) 0);
		// try {
        // // Thuc hien giao dich nap tien
			// short tempBalance = (short) (prevBalance + amount);
        // // Cap nhat giá tri so du moi vào mang da gii mã
			// Util.setShort(decryptedBalance, (short) 0, tempBalance);
        // // Mã hóa so du moi
			// byte[] encryptedBalance = encrypt(decryptedBalance);
        // // Cap nhat giá tri so du moi vào mang OpSoDu và lenSODU
			// Util.arrayCopyNonAtomic(encryptedBalance, (short) 0, OpSoDu, (short) 0, (short) encryptedBalance.length);
			// lenSODU = (byte) encryptedBalance.length;
		// } catch (SystemException e) {
        // Util.arrayCopyNonAtomic(prevEncryptedBalance, (short) 0, OpSoDu, (short) 0, lenSODU);
        // ISOException.throwIt(ISO7816.SW_DATA_INVALID);
		// }
		apdu.setIncomingAndReceive();
		byte[] buffer = apdu.getBuffer();
		lenSODU = buffer[ISO7816.OFFSET_LC];
		byte[] temp = new byte[lenSODU];
		short amount = Util.getShort(buffer,ISO7816.OFFSET_CDATA);
		if(lenSODU> 25)
		{
			ISOException.throwIt(ISO7816.SW_WRONG_LENGTH);
		}else{
			Util.setShort(temp,(short)0,amount); 
			OpSoDu = new byte[lenSODU];
			OpSoDu = encrypt(temp);
		}
	 }
    ...
    // Lu giá tr s d hin ti trc khi thc hin giao dch
		byte[] prevEncryptedBalance = JCSystem.makeTransientByteArray((short) lenSODU, JCSystem.CLEAR_ON_DESELECT);
		Util.arrayCopyNonAtomic(OpSoDu, (short) 0, prevEncryptedBalance, (short) 0, lenSODU);
    ...
    // Ly giá tr s d hin ti t mng ã gii mã
		short prevBalance = Util.getShort(decryptedBalance, (short) 0);

		try {
        // Thc hin giao dch thanh toán
				short tempBalance = (short) (prevBalance - amount);

				if (tempBalance < 0) {
            // S d không   thc hin thanh toán
					ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
					}

        // Cp nht giá tr s d mi vào mng ã gii mã
				Util.setShort(decryptedBalance, (short) 0, tempBalance);

        // Mã hóa s d mi
				byte[] encryptedBalance = encrypt(decryptedBalance);

        // Cp nht giá tr s d mi vào mng OpSoDu và lenSODU
				Util.arrayCopyNonAtomic(encryptedBalance, (short) 0, OpSoDu, (short) 0, (short) encryptedBalance.length);
				lenSODU = (byte) encryptedBalance.length;

        // Gi mã thành công v cho terminal
				Util.arrayCopy(encryptedBalance, (short) 0, buffer, (short) 0, (short) encryptedBalance.length);
				apdu.setOutgoingAndSend((short) 0, (short) encryptedBalance.length);
				} catch (Exception e) {
        // Giao dch b hy do li xy ra
        // Khôi phc s d v giá tr trc khi thc hin giao dch
						Util.arrayCopyNonAtomic(prevEncryptedBalance, (short) 0, OpSoDu, (short) 0, lenSODU);
